...
class Task{
    ...
    public function index($limit = 0){
        ...
        $maxlen = 1024;
        # 查找有上级关系，上级列缓存未完成的用户
        $user = Db::name('member')->field('user_id,first_leader,parents_cache')->where(['parents_cache' => ['=', 0], 'first_leader' => ['>', 0]])->order('first_leader asc')->find();

        if($user){
            if($user['first_leader'] == $user['user_id']){
                Db::name('member')->where('user_id', $user['user_id'])->update(['first_leader' => 0]);
                goto OnceMore;
            }
            # 找到用户顺位上级列
            ...
            if(!$pcache){
                ...
                if($first_parents_cache){
                    ...
                    if($count <= $maxlen){
                        ...
                        if($parents[$count] == 0){
                            Db::name('member')->where('user_id', $user['user_id'])->update(['parents_cache' => 1]);
                        }
                        goto OnceMore;
                    }else{
